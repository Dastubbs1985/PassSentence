...
def main():
	...
	if option == 's':
		...
		filename = user + '.txt'
		createUser(filename)
		print '\nThe sentence you select will be used to generate all of your passwords. \nIt must be a minimum of 20 characters long in order to create all of your required passwords.\nYour chosen sentence requires only letters as the program will add any additional characters required.\nYour letters must be a mixture of upper and lower case letters.'
		x = 0
		while x == 0:
			print '\nPlease enter your PassSentence: '
			global passSentence
			...
			if size >= 25 and len(upperCase) > 0:
				x = 1
			elif size < 25:
				print '\nPassSentence is too small. It only contains ' + str(size) + ' characters.'
			elif len(upperCase) == 0:
				print '\nThere are no capital letters in your PassSentence. Please include at least 1 capital letter.'
		createPassword()
	elif option == 'l':
		x = 0
		while x == 0:
		...
		x = 0
		while x == 0:
			print '\nWould you like to (c)reate a new password or (v)iew your existing passwords? '
			sel = raw_input()
			if sel == 'c':
				x = 1
				createPassword()
				...
				print '\nSelection not recognised. Please enter \'c\' or \'v\' to continue.'
def createPassword():
	print '\nWhat is the website where the generated password will be used? '
	global website 
	website = raw_input()
	size = len(passSentence)
...
def generatePassword(generatedSentence, passSentence, size):
	...
	while y == 0:
		...
		if choice == 'n':
			...
			print '\nChoice not recognised. Please only enter a \'y\' or an \'n\'.'
	x = 0
	while x == 0:
		print '\nWould you like to (c)reate another password, (v)iew your current passwords or (e)xit the program?'
		choice = raw_input()
		if choice == 'c':
			x = 1
			createPassword()
			...
			print '\nChoice not recognised. Please enter a \'c\', \'v\' or \'e\' to continue.'
...
def createUser(filename):
	#  Creates a file using the input username by the user.
